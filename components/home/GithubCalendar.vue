...
export default {
  ...
  mounted() {
    ...
    const init = () => {
      try {
        GitHubCalendar('.calendar', username, {
          ...
          tooltips: true,
          proxy: (user) =>
            fetch(`https://cors.isomorphic-git.org/https://github.com/users/${user}/contributions`)
              .then(r => r.text())
        })
        this.calendarInitialized = true
      } catch (e) {
        // retry once after next tick in case DOM not ready
        this.$nextTick(() => {
          try {
            GitHubCalendar('.calendar', username, {
              responsive: true,
    ...
    this.$nextTick(init)
    // Fallback re-init after CSS/layout settles
    setTimeout(() => {
      if (!this.calendarInitialized) init()
    }, 800)
  }
}
</script>
